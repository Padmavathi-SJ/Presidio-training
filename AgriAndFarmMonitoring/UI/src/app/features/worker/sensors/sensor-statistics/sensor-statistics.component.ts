...
export class SensorStatisticsComponent implements OnInit, OnDestroy {
  ...
  loadStatistics(): void {
    const workerId = this.authService.getCurrentUser()?.id;
    if (!workerId) {
      this.showError('No worker found. Please login again.');
      return;
    }
  ...
  const toDate = filterValues.toDate ? new Date(filterValues.toDate).toISOString() : undefined;
    ...
    this.workerSensorService.getStatistics(groupBy, fromDate, toDate)
