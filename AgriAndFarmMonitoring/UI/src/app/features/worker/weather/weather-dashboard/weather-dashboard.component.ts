...
@Component({
  selector: 'app-worker-weather-dashboard',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class WorkerWeatherDashboardComponent implements OnInit, OnDestroy {
  ...
  private loadActiveAlerts(farmId: number): Promise<void> {
    return new Promise((resolve) => {
      this.weatherService.getWeatherAlerts({ isActive: true })
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (response) => {
            if (response.success) {
              this.activeAlerts.set(response.data?.items || []);
            }
            resolve();
          },
          error: () => resolve()
  ...
  viewAlertDetails(alert: WeatherAlert): void {
    this.dialog.open(WorkerWeatherAlertDialogComponent, {
      width: '600px',
      data: alert,
      panelClass: 'dialog-landscape'
    });
  }

  acknowledgeAlert(alert: WeatherAlert, event: Event): void {
    event.stopPropagation();
    this.weatherService.resolveAlert(alert.id, 'Acknowledged by worker').subscribe({
      next: (res: any) => {
        if (res.success) {
          this.refreshData();
